...
const TradeSimulator = () => {
  ...
  const openPosition = (direction, entryPrice, entryIndex, entryTime, currentPrice = entryPrice) => {
    const series = seriesRef.current;
  ...
  const enterMarket = (direction) => {
    ...
    stopPlayback();
    openPosition(direction, data[idxRef.current].close, idxRef.current, data[idxRef.current].time);
  };
  ...
  const stepFill = () => {
    ...
    series.update(bar);
    followView(i);
    const tagged = p.direction === "long" ? bar.low <= p.limitPrice : bar.high >= p.limitPrice;
    if (tagged) {
  ...
  const runToResolve = () => {
    ...
    maxBarRef.current = Math.min(data.length - 1, activeFromRef.current + MAX_FORWARD);
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      grid: {
        vertLines: { color: dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)" },
        horzLines: { color: dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)" },
      },
      rightPriceScale: { borderColor: dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)" },
      timeScale: {
    ...
    const onMove = (e) => {
      ...
      if (!dragRef.current) {
        const over =
          e.clientX >= rect.left && e.clientX <= rect.right && y >= 0 && y <= rect.height;
        container.style.cursor =
          over && draggables().some((k) => near(y, priceOf(k))) ? "ns-resize" : "default";
        return;
      }
      ...
      if (key === "limit") {
        ...
        const long = t.direction === "long";
        if (key === "tp") {
          price = long ? Math.max(price, t.entryPrice + gap) : Math.min(price, t.entryPrice - gap);
        } else {
          price = long ? Math.min(price, t.entryPrice - gap) : Math.max(price, t.entryPrice + gap);
        }
        t[key] = price;
  ...
  useEffect(() => {
    ...
    chart.applyOptions({
      ...
      grid: {
        vertLines: { color: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)" },
        horzLines: { color: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)" },
      },
      rightPriceScale: { borderColor: isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)" },
      timeScale: { borderColor: isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)" },
    });
  }, [isDark]);
  ...
  const drawBox = () => {
    ...
    const pnl = round2(t.direction === "long" ? cur - t.entryPrice : t.entryPrice - cur);
    const dollars = round2(pnl * DOLLARS_PER_POINT);
    const pos = pnl >= 0;
    box.style.display = "block";
    box.style.top = `${Math.max(10, Math.min(container.clientHeight - 10, coord))}px`;
    ...
    box.textContent = `${pos ? "+" : ""}${pnl} pts  ${pos ? "+" : ""}$${dollars.toLocaleString()}`;
  };
  ...
  const rr =
    trade &&
    round2(
      Math.abs(trade.tp - trade.entryPrice) / Math.abs(trade.entryPrice - trade.sl)
    );
  const resultColor =
    result?.outcome === "win"
      ? "text-green-600"
  ...
  const resultLabel = { win: "WIN", loss: "LOSS", open: "OPEN", nofill: "LIMIT NOT FILLED" }[
    result?.outcome
  ];
  ...
  return (
    <Card className="my-12">
      <CardHeader>
        ...
        <CardDescription>
          Roll a random moment in the last 60 days of Nasdaq-100 futures. Take a market
          trade, or rest a limit order and run the tape until it's tagged — then drag the
          TP/SL lines and run it out to see if you win.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="relative h-[400px] w-full">
          ...
          {!data && (
            <div className="absolute inset-0 flex items-center justify-center text-sm text-muted-foreground">
              {error ? `Failed to load chart data: ${error}` : "Loading NQ futures data…"}
            </div>
          )}
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={newScenario} disabled={!data || running}>
            🎲 New date
          </Button>
          <Button
            size="sm"
          ...
          <Button variant="ghost" size="sm" onClick={resetTrade} disabled={idle || running}>
            Reset
          </Button>
        ...
        {entryInfo && (
          <div className="text-sm text-muted-foreground">
            Date: <span className="font-medium">{fmtDate(entryInfo.time)}</span> · price{" "}
            {entryInfo.price}
          </div>
        )}
        ...
        {pending && phase === "pending" && (
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm">
            <span className="text-blue-600">
              Pending {pending.direction === "long" ? "BUY" : "SELL"} LIMIT @ {pending.limitPrice}
            </span>
            <span className="text-muted-foreground">drag the blue line, then Run to fill</span>
          </div>
        )}
        ...
        {trade && (
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm">
            <span>
              Direction: <span className="font-semibold uppercase">{trade.direction}</span>
            </span>
          ...
          <div className={`text-sm font-semibold ${resultColor}`}>
            {result.outcome === "nofill" ? (
              ...
              <>
                ...
                {round2(result.pnl * DOLLARS_PER_POINT).toLocaleString()}) · exit{" "}
                {result.exitPrice} @ {fmtDate(result.exitTime)}
              </>
            )}
          </div>
        )}
